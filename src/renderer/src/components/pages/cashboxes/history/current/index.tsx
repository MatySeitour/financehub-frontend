...
import { CurrentCommissionsHistoryCashbox } from "@renderer/components/cashboxes/histories/current/commissions";
...
export function HistoryCurrentSection() {
  ...
  >({
    queryKey: ["summary"],
    queryFn: () =>
      getSummary(openingDateTime ? new Date(openingDateTime) : new Date()),
    enabled: !!openingDateTime,
  });
        ...
        <article className="flex h-auto w-full flex-col gap-4 overflow-hidden py-4">
          ...
          <div className="flex h-auto min-h-0 flex-col gap-4 px-4">
            ...
            {tabActive === "commissions" && (
              <CurrentCommissionsHistoryCashbox cashboxID={cashboxID} />
            )}
          </div>
        </article>
      )}
