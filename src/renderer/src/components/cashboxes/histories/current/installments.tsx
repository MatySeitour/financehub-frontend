...
export function CurrentInstallmentsHistoryCashbox({
  ...
  const historyInstallmentsQuery = useQuery<
    Awaited<ReturnType<typeof getCashboxCurrentHistoryInstallments>>,
    BaseResponseServer
  >({
    queryKey: ["history-installments-current", cashboxID],
    queryFn: () => getCashboxCurrentHistoryInstallments(cashboxID ?? -1),
    retry: false,
    enabled: !!cashboxID,
  });
